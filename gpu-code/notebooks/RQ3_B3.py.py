...
high_level_features = ['was_aware_num',  'posing_num',  'comfort_num',  'will_num', 'photographer_intention_num',
...
def do_cross_validation_mixed_features(model_func, img_feat, other_feat, Y, n_splits, save_model = True, save_prefix = ''):
    seed = 1234
    kfold = StratifiedKFold(n_splits=n_splits, shuffle=True, random_state=seed)
    cvscores = []
...
scores3 = do_cross_validation_mixed_features(resnet_mixed_features,
        img_feat=feature_df.resized_cropped_img, 
                                    other_feat=feature_df[high_level_features].apply(stats.zscore),
                                    Y=feature_df.label, n_splits = 2, save_prefix = 'high_level_features')
#clear_output()
print("%.2f%% (+/- %.2f%%)" % (np.mean(scores), np.std(scores)))
